...
// Add Credit Ratings and short interest
preserve
	keep if gvkey == .
...
// Add Gompers et al Governance index
preserve
	keep if mi(ticker)
	save temp, replace
restore
...
drop if mi(ticker)
bysort ticker time_avail_m: keep if _n == 1  // A tiny number of obs has duplicates in a month
...
// Add monthly version of quarterly Compustat
preserve
	keep if gvkey == .
	save temp, replace  // Temporarily store obs with missing gvkeys in another file
restore
...
drop if mi(ticker)
bysort ticker time_avail_m: keep if _n == 1  // A tiny number of obs has duplicates in a month
merge 1:1 ticker time_avail_m using "$pathProject/DataClean/OptionMetrics", keep(master match) nogenerate
append using temp
...
// Add Customer and Supplier Momentum
preserve
	keep if gvkey == .
	save temp, replace  // Temporarily store obs with missing gvkeys in another file
restore
