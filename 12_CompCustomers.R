...
seg_customer = data.table::fread(file = '../DataRaw/CompustatSegmentDataCustomers.csv') %>% 
  mutate(datadate = ymd(datadate))

ccm          = data.table::fread(file = '../DataRaw/CCMLinkingTable.csv') %>% 
  mutate(linkdt    = ymd(linkdt),
         linkenddt = ymd(linkenddt))

m_crsp       = data.table::fread(file = '../DataRaw/mCRSP.csv') %>% 
...
seg_customer2 = seg_customer %>% 
  ...
  filter(!is.na(datadate),!is.na(cust_permno),
         (datadate >= linkdt) & (datadate <= linkenddt | is.na(linkenddt))) %>%
...
## testing: this stuff doesn't work but it's fun to try maybe later
# check nas

## temp = temp2 %>% mutate(matched = !is.na(cust_permno)) %>% group_by(matched) %>% summarize(n = n())
## temp = temp2 %>% filter(is.na(cust_permno)) %>% distinct(cnms) %>% arrange(cnms)


## library(fuzzyjoin)
## hardmatch = temp2 %>% filter(is.na(cust_permno) & !is.na(cnms)) %>% select(cnms) %>% distinct %>% arrange(cnms)

## allconm = ccm0 %>% select(conm) %>% filter(!is.na(conm)) %>% mutate(conm = toupper(conm)) %>% distinct %>% arrange(conm)

## # soundex takes 3 min
## ptm <- proc.time()
## temp2b = stringdist_left_join(
##     hardmatch
##     ,allconm
##     ,by =c("cnms"="conm")
##     ,method = "osa" # osa, soundex
##     ,distance_col = "dist"
##     ,max_dist = 1 # default = 2
##     )
## proc.time() - ptm    
## temp = temp2b %>% arrange(cnms,dist) %>% select(cnms,conm,dist)
...
## write to disk
data.table::fwrite(customerMom, file = '../DataRaw/customerMom.csv') 


## ### TESTING ###
## ac: I might want to come back later to see if I can improve the matching and thus performance
## tempc = m_crsp 
## day(tempc$date) = 28
## tempc = tempc %>% mutate(mve_c = abs(prc)*shrout) %>%  filter(abs(prc) > 5)


## temp1 = inner_join(tempc,customerMom,by=c("permno","date"="time_avail_m")) %>%
##     select(permno,date,ret,CustMom,mve_c) %>%
##     group_by(permno) %>%
##     mutate(ret1pct = 100*lead(ret,order_by=date))

## temp2 = temp1 %>%
##   group_by(date) %>% 
##   mutate(Quantile = findInterval(CustMom, 
##                 quantile(CustMom, probs=0:5/5)
##                 )
##          )

## tempport = temp2 %>% group_by(date,Quantile) %>%
##     filter(!is.na(mve_c) & !is.na(ret1pct)) %>%    
##     summarize(    
##        ret.ew = mean(ret1pct,na.rm=T)
##        ,ret.vw = weighted.mean(ret1pct,w=mve_c,na.rm=T)     
##        ,n=n()
##     )

## tempsum = tempport %>% group_by(Quantile) %>%
##     filter(year(date)>= 1981) %>%
##     filter(year(date)<= 2004) %>%    
##     summarize(
##        sd.ew = sd(ret.ew,na.rm=T)        
##        ,ret.ew = mean(ret.ew,na.rm=T)
##        ,ret.vw = mean(ret.vw,na.rm=T)       
##        ,n=sum(n)
##        ,T = n()
##               ) %>%
##     mutate(
##         t.ew = ret.ew/sd.ew*sqrt(T)
##         )
## temp = tempsum
...
## tempw = tempport %>%
##     select(-c(ret.ew,n)) %>%
##     spread(Quantile,ret.vw,sep="") %>%
##     mutate(LS = Quantile5-Quantile1) %>%
##     filter(year(date)>= 1981) %>%
##     filter(year(date)<= 2004)   

## ret = tempw$LS[which(!is.na(tempw$LS))]
## mean(ret)
...
## ## other stats
## # cohen and frazzini have at least 390 obs, we have a minimum of 202
## temp = customerMom %>% mutate(year=year(time_avail_m)) %>%
##     group_by(year) %>% summarize(n=n()/12)
