...
ggsave(device = cairo_pdf, filename = paste0(pathResults, "fig2b_reprate_PredictorPlacebo_Jitter.pdf"), width = 10, height = 8)
...
df_tmp = stats %>%
    ...
    transmute(
        ...
        Cat.Signal,
        inMP = signalname %in% mpSignals$ourname,
        inHXZ = signalname %in% hxzSignals$ourname) 
...
ggsave(device = cairo_pdf, filename = paste0(pathResults, 'fig_reprate_ourstudy.pdf'), width = 12, height = 8)
...
ggsave(device = cairo_pdf, filename = paste0(pathResults, 'fig_reprate_HXZ.pdf'), width = 12, height = 8)
...
ggsave(device = cairo_pdf, filename = paste0(pathResults, 'fig_reprate_MP.pdf'), width = 12, height = 8)
