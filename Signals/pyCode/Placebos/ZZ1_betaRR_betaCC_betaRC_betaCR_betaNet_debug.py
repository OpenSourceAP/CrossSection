...
def main():
    ...
    ########################CHECKPOINT 2b##################################
    #%%
    stata_df = pd.read_stata("../../pyData/Debug/checkpoint2b.dta")
    stata_df.head()
    #%%
    stata_df = stata_df[['time_avail_m', 'MarketIlliquidity', 'rM', 'MarketCapitalization']]
    id_cols = [c for c in ['permno', 'date', 'time_avail_m'] if c in stata_df.columns]
    stata_df_long = (
        stata_df.melt(id_vars=id_cols, var_name='variable', value_name='value')
                .sort_values(id_cols + ['variable'])
    ...
    market_subset.head()
    #%%
    python_df = market_subset[['time_avail_m', 'MarketIlliquidity', 'rM', 'MarketCapitalization']]
    id_cols = [c for c in ['permno', 'date', 'time_avail_m'] if c in python_df.columns]
    python_df_long = (
        python_df.melt(id_vars=id_cols, var_name='variable', value_name='value')
                .sort_values(id_cols + ['variable'])
    ...
    ########################CHECKPOINT 2##################################
    #%%
    stata_df = pd.read_stata("../../pyData/Debug/checkpoint2c.dta")
    stata_df.head()
    #%%
    stata_df = stata_df[['time_avail_m', 'temp', 'templ1', 'templ2']]
    id_cols = [c for c in ['permno', 'date', 'time_avail_m'] if c in stata_df.columns]
    stata_df_long = (
        stata_df.melt(id_vars=id_cols, var_name='variable', value_name='value')
                .sort_values(id_cols + ['variable'])
    ...
    stata_df_long = stata_df_long[(stata_df_long["time_avail_m"] > pd.Timestamp("2015-01-01"))].copy()
    stata_df_long.rename(columns={'value': 'stata'}, inplace=True)
    stata_df_long

    #%%
    python_df = market_agg[['time_avail_m', 'temp', 'templ1', 'templ2']]
    id_cols = [c for c in ['permno', 'date', 'time_avail_m'] if c in python_df.columns]
    python_df_long = (
        python_df.melt(id_vars=id_cols, var_name='variable', value_name='value')
                .sort_values(id_cols + ['variable'])
    ...
    both = both.sort_values(by="diff", ascending=False, na_position="first")
    both
    both.to_csv("../../pyData/Debug/both_checkpoint2c.csv", index=False)
    #%%
    ####################################################################

    # Manual rolling regression to exactly match Stata sequential process
    print("Running manual rolling regressions to match Stata exactly...")
    import statsmodels.api as sm
    
    # Initialize coefficient and residual columns
    market_agg['APa0'] = np.nan
    market_agg['APa1'] = np.nan  
    market_agg['APa2'] = np.nan
    ...
    for i in range(len(market_agg)):
        # Get 60-month window ending at current observation (inclusive)
        start_idx = max(0, i - 59)  # 60 months including current
        window_data = market_agg.iloc[start_idx:i+1].copy()
        
        # First regression: illiquidity (temp ~ templ1 + templ2 + constant)
        reg_vars_ill = ['temp', 'templ1', 'templ2']
        window_valid_ill = window_data.dropna(subset=reg_vars_ill)
        
        if len(window_valid_ill) >= 48:  # min(48) observations
            try:
                X_ill = sm.add_constant(window_valid_ill[['templ1', 'templ2']])
                y_ill = window_valid_ill['temp']
                
                model_ill = sm.OLS(y_ill, X_ill).fit()
                
                # Store coefficients (lines 51-53)
                market_agg.iloc[i, market_agg.columns.get_loc('APa0')] = model_ill.params['const']
                market_agg.iloc[i, market_agg.columns.get_loc('APa1')] = model_ill.params['templ1'] 
                market_agg.iloc[i, market_agg.columns.get_loc('APa2')] = model_ill.params['templ2']
                
                # Calculate residual for current observation (line 50: rename _residuals eps_c_M)
                current_temp = market_agg.iloc[i]['temp']
                current_templ1 = market_agg.iloc[i]['templ1']
                current_templ2 = market_agg.iloc[i]['templ2']
                
                if not (pd.isna(current_temp) or pd.isna(current_templ1) or pd.isna(current_templ2)):
                    fitted_temp = (model_ill.params['const'] + 
                                 model_ill.params['templ1'] * current_templ1 + 
                                 model_ill.params['templ2'] * current_templ2)
                    market_agg.iloc[i, market_agg.columns.get_loc('eps_c_M')] = current_temp - fitted_temp
                    
            except:
                pass  # Leave as NaN if regression fails
    
    # Create separate lag variables exactly as in Stata (lines 56-57)
    # gen tempRlag1 = l.rM  
    # gen tempRlag2 = l2.rM
    market_agg['tempRlag1'] = market_agg['l1_rM'] 
    market_agg['tempRlag2'] = market_agg['l2_rM']
    ...
    for i in range(len(market_agg)):
        ...
        # Second regression: return (rM ~ tempRlag1 + tempRlag2 + constant)
        reg_vars_ret = ['rM', 'tempRlag1', 'tempRlag2']
        window_valid_ret = window_data.dropna(subset=reg_vars_ret)
        
        if len(window_valid_ret) >= 48:  # min(48) observations
            try:
                X_ret = sm.add_constant(window_valid_ret[['tempRlag1', 'tempRlag2']])
                y_ret = window_valid_ret['rM']
                
                model_ret = sm.OLS(y_ret, X_ret).fit()
                
                # Calculate residual for current observation (line 59: rename _residuals eps_r_M)
                current_rM = market_agg.iloc[i]['rM']
                current_tempRlag1 = market_agg.iloc[i]['tempRlag1']
                current_tempRlag2 = market_agg.iloc[i]['tempRlag2']
                
                if not (pd.isna(current_rM) or pd.isna(current_tempRlag1) or pd.isna(current_tempRlag2)):
                    fitted_rM = (model_ret.params['const'] + 
                               model_ret.params['tempRlag1'] * current_tempRlag1 + 
                               model_ret.params['tempRlag2'] * current_tempRlag2)
                    market_agg.iloc[i, market_agg.columns.get_loc('eps_r_M')] = current_rM - fitted_rM
