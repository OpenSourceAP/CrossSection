...
proc import datafile = '~/temp_prep/data_for_dl/issm_monthly.csv' out=issm 
	dbms=csv replace;
	guessingrows=6000;
run;	

proc import datafile = '~/temp_prep/data_for_dl/wrds_iid_monthly.csv' out=iid 
	dbms=csv replace;
	guessingrows=6000;
run;	
...
proc export data = hf3
  outfile = "~/temp_prep/data_for_dl/hf_monthly.csv"
  dbms = csv
  replace;
run;
