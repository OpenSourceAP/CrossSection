...
def asrol_fast(
    ...
    if not consecutive_only:
        ...
        result_pl = _simple_rolling(
            df_pl, group_col, value_col, window, stat, new_col_name, min_periods
        )
    else:
...
def asrol_calendar(
    ...
    if require_prev_obs:
        df_with_prev = df_with_prev.with_columns(
            ...
            [
                ...
                ).alias("has_valid_prev_obs")
            ]
        )

    df_addition = (
        df_with_prev.with_columns(pl.col([group_col, date_col]).set_sorted())
        .rolling(index_column=date_col, period=window, group_by=group_col)
        .agg(
            [
            ...
            + (
                [pl.last("has_valid_prev_obs").alias("has_valid_prev_obs")]
                if require_prev_obs
                else []
            )
        )
    ...
    if require_prev_obs:
        ...
        df_addition = df_addition.with_columns(
            pl.when(pl.col(f"{value_col}_obs") >= min_obs)
            .then(pl.col(f"{value_col}_{stat}"))
            .otherwise(pl.lit(None))
            .alias(f"{value_col}_{stat}")
        )
...
def asrol_calendar_pd(
    ...
    result_pl = asrol_calendar(
        df_pl, group_col, date_col, value_col, stat, window, min_obs, require_prev_obs
    )
