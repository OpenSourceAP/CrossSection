...
df = pd.read_parquet('../pyData/Intermediate/monthlyCRSP.parquet',
    columns=['permno', 'ticker', 'exchcd', 'shrcd', 'time_avail_m', 'mve_c', 'mve_permco', 'prc', 'ret', 'sicCRSP'])
...
if IBESCRSPLink_path.exists():
    ...
    ibes_link = pd.read_parquet(
        IBESCRSPLink_path, columns=["permno", "time_avail_m", "tickerIBES"]
    )
...
# Reorder columns
df = df[
    ['permno', 'time_avail_m'] # main ids
    + ['gvkey','tickerIBES','secid'] # secondary ids
    + ['exchcd','shrcd','NYSE'] # exchange and share class info
    + ['sicCRSP','sicCS'] # sic codes
    + ['prc','ret','mve_c','mve_permco'] # commonly used crsp variables
    + ['ticker'] # ticker TBC: we should remove this. I think it's only used in the governance index stuff
]
