...
# Prompt for cleanup
read -p "Clean remote directory ~/temp_prep before upload? (y/n): " CLEANUP

# Clean remote directory if requested
if [ "$CLEANUP" == "y" ] || [ "$CLEANUP" == "Y" ]; then
    echo "Cleaning remote directory ~/temp_prep"
    sshpass -p "$WRDS_PASSWORD" ssh -o StrictHostKeyChecking=no "$WRDS_USERNAME@wrds-cloud.wharton.upenn.edu" "rm -r -f ~/temp_prep/"
fi
