# ABOUTME: Translates UpRecomm.do to calculate recommendation upgrades indicator
# ABOUTME: Run with: python3 Predictors/UpRecomm.py

# Calculates binary indicator for analyst recommendation upgrades using IBES data
# Input: ../pyData/Intermediate/IBES_Recommendations.parquet, ../pyData/Intermediate/SignalMasterTable.parquet  
# Output: ../pyData/Predictors/UpRecomm.csv
...
# bys tickerIBES (time_avail_m): gen UpRecomm = ireccd < ireccd[_n-1] & ireccd[_n-1] != .
...
# Calculate UpRecomm: current < previous AND previous is not missing (upgrade = lower number)
df['UpRecomm'] = ((df['ireccd'] < df['ireccd_lag']) & (df['ireccd_lag'].notna())).astype(int)

# CHECKPOINT 3: After creating UpRecomm variable
aapl_data = df[(df['tickerIBES'] == 'AAPL') & (df['time_avail_m'] == pd.Timestamp('1993-11-01'))]
if not aapl_data.empty:
    print("CHECKPOINT 3 - After creating UpRecomm variable:")
    print(aapl_data[['tickerIBES', 'time_avail_m', 'ireccd', 'UpRecomm']])
...
result = result[['permno', 'yyyymm', 'UpRecomm']]
...
# SAVE
result.to_csv('../pyData/Predictors/UpRecomm.csv', index=False)

print(f"UpRecomm predictor created with {len(result)} observations")