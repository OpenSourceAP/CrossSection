...
compustat_df = pd.read_parquet("../pyData/Intermediate/m_aCompustat.parquet", 
                               columns=['permno', 'time_avail_m', 'at', 'dltt', 'dlc', 'pstk', 'dvpa', 'tstkp', 'che', 'sic', 'ib', 'csho', 'ceq', 'prcc_f'])

# Remove duplicates by permno time_avail_m (keep first)
compustat_df = compustat_df.drop_duplicates(subset=['permno', 'time_avail_m'], keep='first')
