...
temp = (df["timeLinkStart_d"] <= df["time_d"]) & (
    (df["time_d"] <= df["timeLinkEnd_d"]) | df["timeLinkEnd_d"].isna()
)
...
df["time_ann_d"] = np.where(
    df["anndat"] == 1, df.index.get_level_values("time_temp"), df["time_ann_d"]
)
...
df["time_ann_d"] = np.where(
    df["anndat_f1"] == 1, df.index.get_level_values("time_temp") + 1, df["time_ann_d"]
)
df["time_ann_d"] = np.where(
    df["anndat_f2"] == 1, df.index.get_level_values("time_temp") + 2, df["time_ann_d"]
)
df["time_ann_d"] = np.where(
    df["anndat_l1"] == 1, df.index.get_level_values("time_temp") - 1, df["time_ann_d"]
)
...
df = (
    df.groupby(["permno", "time_ann_d"])
    .agg({"AnnouncementReturn": "sum", "time_d": "max"})
    .reset_index()
)
...
df = (
    df.sort_values(["permno", "time_avail_m"])
    .groupby(["permno", "time_avail_m"])
    .tail(1)
)
...
for permno in df.index.get_level_values("permno").unique():
    ...
    full_range = pd.period_range(
        start=permno_df.index.min(), end=permno_df.index.max(), freq="M"
    )
