...
df = df.merge(df_revt.rename(columns={'time_avail_m': 'time_lag12_revt', 'revt': 'revt_lag12'}),
              on=['permno', 'time_lag12_revt'], how='left')
df = df.drop('time_lag12_revt', axis=1)

# Replicate Stata's log behavior: log of zero/negative is missing
valid_mask = (df['revt'] > 0) & (df['revt_lag12'] > 0) & df['revt'].notna() & df['revt_lag12'].notna()
df['temp'] = np.nan
df.loc[valid_mask, 'temp'] = np.log(df.loc[valid_mask, 'revt']) - np.log(df.loc[valid_mask, 'revt_lag12'])
...
df = df.merge(df_ranks.rename(columns={'time_avail_m': 'time_lag12', 'tempRank': 'tempRank_lag12'}),
              on=['permno', 'time_lag12'], how='left')
df = df.merge(df_ranks.rename(columns={'time_avail_m': 'time_lag24', 'tempRank': 'tempRank_lag24'}),
              on=['permno', 'time_lag24'], how='left')
df = df.merge(df_ranks.rename(columns={'time_avail_m': 'time_lag36', 'tempRank': 'tempRank_lag36'}),
              on=['permno', 'time_lag36'], how='left')
df = df.merge(df_ranks.rename(columns={'time_avail_m': 'time_lag48', 'tempRank': 'tempRank_lag48'}),
              on=['permno', 'time_lag48'], how='left')
df = df.merge(df_ranks.rename(columns={'time_avail_m': 'time_lag60', 'tempRank': 'tempRank_lag60'}),
              on=['permno', 'time_lag60'], how='left')

# Clean up temporary columns
df = df.drop(['time_lag12', 'time_lag24', 'time_lag36', 'time_lag48', 'time_lag60'], axis=1)
