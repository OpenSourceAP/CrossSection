...
compustat = compustat[
    ["permno", "time_avail_m", "xrd", "sich", "datadate", "ceq"]
].copy()
...
df["tempCitationsRD"] = np.where(
    df["sum_xrd"] > 0, df["sum_ncit"] / df["sum_xrd"], np.nan
)
...
df = df.assign(
    sicCRSP=lambda x: x["sicCRSP"].fillna(np.inf)
)  # treat missing SIC codes as non-financial
...
nyse_medians = (
    df[df["exchcd"] == 1]
    .groupby("time_avail_m")["mve_c"]
    .apply(lambda x: stata_quantile(x, 0.5))
    .reset_index()
)
