...
df = df[['permno', 'time_avail_m', 'ivao', 'ivst', 'dltt', 'dlc', 'pstk', 'sstk', 'prstkc', 'dv', 
         'act', 'che', 'lct', 'at', 'lt', 'ni', 'oancf', 'ivncf', 'fincf']].copy()
...
df['tempNc'] = (df['at'] - df['act'] - df['tempivao']) - (df['lt'] - df['tempdlc'] - df['tempdltt'])
df['tempFi'] = (df['tempivst'] + df['tempivao']) - (df['tempdltt'] + df['tempdlc'] + df['temppstk'])
...
df.loc[early_years, 'TotalAccruals'] = ((df['tempWc'] - df['tempWc_lag12']) + 
                                        (df['tempNc'] - df['tempNc_lag12']) + 
                                        (df['tempFi'] - df['tempFi_lag12']))

# For years > 1989, use cash flow method
late_years = df['year'] > 1989
df.loc[late_years, 'TotalAccruals'] = (df['ni'] - (df['oancf'] + df['ivncf'] + df['fincf']) + 
