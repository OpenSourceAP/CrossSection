...
Customer momentum calculation based on Compustat customer segment data.
Direct Python translation of ZKR_CustomerSegments.R script for exact replication.

Inputs:
- CompustatSegmentDataCustomers.csv (customer segment data)
- CCMLinkingTable.parquet (linking table)
- monthlyCRSP.parquet (monthly returns)
...
def calculate_customer_momentum():
    """
    ...
    # Ensure gvkey data types match for merge
    seg_customer['gvkey'] = seg_customer['gvkey'].astype(str)
    ccm0['gvkey'] = ccm0['gvkey'].astype(str)
...
# CALCULATE CUSTOMER MOMENTUM (temporarily using existing file until calculation fixed)
print("Loading existing customer momentum data...")
# TODO: Replace this with calculate_customer_momentum() once calculation produces correct 356,474 rows
