...
monthly_crsp = pl.read_parquet("../pyData/Intermediate/monthlyCRSP.parquet").select(
...
df_with_beta = df_with_beta.with_columns(pl.col("b_tailex").alias("BetaTailRisk"))

# Apply filters: remove missing betas and non-common stocks (shrcd > 11)
df_final = df_with_beta.filter(
    pl.col("BetaTailRisk").is_not_null() & (pl.col("shrcd") <= 11)
