...
df = df.with_columns(
    pl.when(pl.col("mve_permco") <= pl.col("p20")).then(1)
    .when(pl.col("mve_permco") <= pl.col("p40")).then(2)
    .when(pl.col("mve_permco") <= pl.col("p60")).then(3)  
    .when(pl.col("mve_permco") <= pl.col("p80")).then(4)
    .otherwise(5)
    .alias("sizecat")
)
