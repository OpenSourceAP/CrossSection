...
first_div_obs = (
    df[df["cd3"].notna()].groupby("permno")["time_avail_m"].min().reset_index()
)
...
df["temp3"] = (
    ...
    & (
        ...
        | stata_ineq_pd(df["divpaid_lag11"], ">", 0)
    )
).astype(int)
...
df["temp5"] = ((df["cd3"] == 5) & stata_ineq_pd(df["divpaid_lag11"], ">", 0)).astype(
    int
)
