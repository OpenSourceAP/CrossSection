...
liquidity = pl.read_parquet("../pyData/Intermediate/monthlyLiquidity.parquet").select(
    ["time_avail_m", "ps_innov"]
)
...
# SIGNAL CONSTRUCTION
print(
    "🧮 Computing Pastor-Stambaugh liquidity beta using direct polars-ols rolling 60-observation 4-factor regressions..."
)
...
print(
    f"Computing rolling 4-factor regressions for {df['permno'].n_unique():,} unique permnos..."
)
...
df_final = (
    df_with_beta.filter(pl.col("b_ps_innov").is_not_null()).select(
        ["permno", "time_avail_m", pl.col("b_ps_innov").alias("BetaLiquidityPS")]
    )
).filter(pl.col("BetaLiquidityPS").is_not_null())
