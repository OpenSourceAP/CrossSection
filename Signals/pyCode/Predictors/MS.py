...
df = (
    compustat.join(smt, on=["permno", "time_avail_m"], how="inner")
...
df = (
    df.with_columns(pl.col("sicCRSP").cast(pl.Utf8).str.slice(0, 2).alias("sic2D"))
    .with_columns(pl.len().over(["sic2D", "time_avail_m"]).alias("sic2D_count"))
    .filter(pl.col("sic2D_count") >= 3)
)
...
df = df.with_columns([pl.col("xad").fill_null(0.0), pl.col("xrdq").fill_null(0.0)])
...
for col in ["niqsum", "xrdqsum", "oancfqsum", "capxqsum"]:
    df = df.with_columns(pl.col(col) * 4)
...
df = df.with_columns(
    [pl.lit(0).alias("m1"), pl.lit(0).alias("m2"), pl.lit(0).alias("m3")]
)

df = df.with_columns(
    [
        pl.when(stata_ineq_pl(pl.col("roa"), ">", pl.col("md_roa")))
        .then(pl.lit(1))
        .otherwise(pl.col("m1"))
        .alias("m1"),
        pl.when(stata_ineq_pl(pl.col("cfroa"), ">", pl.col("md_cfroa")))
        .then(pl.lit(1))
        .otherwise(pl.col("m2"))
        .alias("m2"),
        pl.when(stata_ineq_pl(pl.col("oancfqsum"), ">", pl.col("niqsum")))
        .then(pl.lit(1))
        .otherwise(pl.col("m3"))
        .alias("m3"),
    ]
...
df = df.with_columns([pl.lit(0).alias("m4"), pl.lit(0).alias("m5")])

df = df.with_columns(
    [
        pl.when(stata_ineq_pl(pl.col("niVol"), "<", pl.col("md_niVol")))
        .then(pl.lit(1))
        .otherwise(pl.col("m4"))
        .alias("m4"),
        pl.when(stata_ineq_pl(pl.col("revVol"), "<", pl.col("md_revVol")))
        .then(pl.lit(1))
        .otherwise(pl.col("m5"))
        .alias("m5"),
    ]
...
df = df.with_columns(
    [
        ...
        (pl.col("xad") / pl.col("atdenom2")).alias(
            "xadint"
        ),  # I can't find xadq or xady
    ]
)
...
df = df.with_columns(
    [pl.lit(0).alias("m6"), pl.lit(0).alias("m7"), pl.lit(0).alias("m8")]
)

df = df.with_columns(
    [
        pl.when(stata_ineq_pl(pl.col("xrdint"), ">", pl.col("md_xrdint")))
        .then(pl.lit(1))
        .otherwise(pl.col("m6"))
        .alias("m6"),
        pl.when(stata_ineq_pl(pl.col("capxint"), ">", pl.col("md_capxint")))
        .then(pl.lit(1))
        .otherwise(pl.col("m7"))
        .alias("m7"),
        pl.when(stata_ineq_pl(pl.col("xadint"), ">", pl.col("md_xadint")))
        .then(pl.lit(1))
        .otherwise(pl.col("m8"))
        .alias("m8"),
    ]
...
for v in ["tempMS"]:
    ...
    df = df.with_columns(pl.col(v).forward_fill().over("permno").alias(v))

# Create final MS score
df = df.with_columns(pl.col("tempMS").alias("MS"))
...
df = df.with_columns(
    [pl.when(pl.col("tempMS") <= 1).then(pl.lit(1)).otherwise(pl.col("MS")).alias("MS")]
)
