...
iomom_collapsed = (
    iomom.groupby(["gvkey", "time_avail_m", "type"])
    .agg({"retmatch": "mean", "portind": "mean"})
    .reset_index()
)
...
iomom_wide.columns = [
    f"{col[0]}{col[1]}" if col[1] else col[0] for col in iomom_wide.columns
]
...
for col in ["retmatchcustomer", "retmatchsupplier"]:
    if col in final_iomom.columns:
        ...
        if len(series_data) > 0:
            logger.info(
                f"{col}: mean={series_data.mean():.6f}, std={series_data.std():.6f}, count={len(series_data)}"
            )
