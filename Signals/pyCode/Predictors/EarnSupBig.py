...
df["EarningsSurprise"] = (
    df.groupby("permno")["epspxq"].transform(lambda x: x - x.shift(12)) - df["Drift"]
)
...
for n in range(3, 25, 3):  # 3, 6, 9, 12, 15, 18, 21, 24
    df[f"temp{n}"] = df.groupby("permno")["EarningsSurprise"].transform(
        lambda x: x.shift(n)
    )
...
industry_earnings = (
    df_big.groupby(["tempFF48", "time_avail_m"])["EarningsSurprise"]
    .mean()
    .reset_index()
)
...
print(
    f"Final dataset has {len(df)} observations with {df['EarnSupBig'].notna().sum()} non-missing EarnSupBig values"
)
