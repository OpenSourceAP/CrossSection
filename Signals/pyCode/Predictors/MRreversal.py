...
for lag in [13, 14, 15, 16, 17, 18]:
    ...
    lag_df = lag_df.rename(columns={'ret': f'ret_lag{lag}', 'ret_orig_missing': f'ret_lag{lag}_orig_missing'})
    
    # Merge to get the lagged values
    df = df.merge(lag_df[['permno', 'time_avail_m', f'ret_lag{lag}', f'ret_lag{lag}_orig_missing']], 
