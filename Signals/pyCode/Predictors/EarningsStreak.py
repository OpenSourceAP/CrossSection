...
df["anndats_act_month"] = (
    pd.to_datetime(df["anndats_act"]).dt.to_period("M").dt.start_time
)
df["month_diff"] = (
    df["time_avail_m"].dt.to_period("M") - df["anndats_act_month"].dt.to_period("M")
).apply(lambda x: x.n)
...
if len(df) < initial_count:
    print(
        f"Warning: Dropped {initial_count - len(df)} observations with missing EarningsStreak after forward-fill"
    )
