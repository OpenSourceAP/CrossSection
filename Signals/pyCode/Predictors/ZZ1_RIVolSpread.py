...
df_bh["time_avail_m"] = (
    pd.to_datetime(df_bh["date"]).dt.to_period("M").dt.to_timestamp()
)
...
df_pivot = df_bh.pivot_table(
    index=["secid", "time_avail_m"], columns="cp_flag", values="impvol", aggfunc="first"
).reset_index()
...
df_pivot.loc[df_pivot["impvol"].isna() & df_pivot["impvolC"].notna(), "impvol"] = (
    df_pivot["impvolC"]
)

# Use put volatility if average is missing but put data exists
df_pivot.loc[df_pivot["impvol"].isna() & df_pivot["impvolP"].notna(), "impvol"] = (
    df_pivot["impvolP"]
)
