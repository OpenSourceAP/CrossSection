...
tr_13f = pd.read_parquet(
    "../pyData/Intermediate/TR_13F.parquet",
    columns=["permno", "time_avail_m", "instown_perc"],
)
df = pd.merge(
    signal_master, tr_13f, on=["permno", "time_avail_m"], how="left", validate="1:1"
)
...
df = pd.merge(
    df, monthly_crsp, on=["permno", "time_avail_m"], how="left", validate="1:1"
)
...
# SAVE
print(
    f"Calculated IO_ShortInterest for {df['IO_ShortInterest'].notna().sum()} observations"
)
