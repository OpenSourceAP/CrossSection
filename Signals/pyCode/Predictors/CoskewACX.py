...
daily_crsp = daily_crsp.filter(pl.col("time_d") >= pl.date(1962, 7, 2))
...
merged_data = daily_crsp.join(
    daily_ff.select(["time_d", "mktrf", "rf"]), on="time_d", how="inner"
).filter(pl.col("mktrf").is_not_null() & pl.col("rf").is_not_null())
...
merged_data = merged_data.with_columns([(pl.col("mktrf") + pl.col("rf")).alias("mkt")])
...
for m in range(1, 13):
    ...
    batch_df = batch_df.with_columns(
        [
            ...
            .then(
                (pl.col("time_d").dt.year() - 1960) * 12
                + (pl.col("time_d").dt.month() - 1)
            )
            .otherwise(None)
            .alias("time_avail_m")
        ]
    ...
    coskew_df = coskew_df.with_columns(
        [
            (pl.col("E_ret_mkt2") / (pl.col("E_ret2").sqrt() * pl.col("E_mkt2"))).alias(
                "CoskewACX"
            )
        ]
    )
    ...
    coskew_df = coskew_df.filter((pl.col("max_nobs") - pl.col("nobs")) <= 5)
...
final_result = (
    ...
    .with_columns([(pl.col("yyyy") * 100 + pl.col("mm")).alias("yyyymm")])
    .select(["permno", "yyyymm", "CoskewACX"])
)
