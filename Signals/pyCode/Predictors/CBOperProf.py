...
signal_master = pd.read_parquet(
    ...
    columns=["permno", "gvkey", "time_avail_m", "exchcd", "sicCRSP", "shrcd", "mve_c"],
)
...
for var in lag_vars:
    ...
    var_lag_data = var_lag_data.rename(
        columns={"time_avail_m": "lag_time", var: f"l12_{var}"}
    )
...
print(
    f"Generated CBOperProf values for {df['CBOperProf'].notna().sum():,} observations"
)
