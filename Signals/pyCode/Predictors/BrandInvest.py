...
df = pd.read_parquet('../pyData/Intermediate/a_aCompustat.parquet', 
                    columns=['gvkey', 'permno', 'time_avail_m', 'fyear', 'datadate', 
                            'xad', 'xad0', 'at', 'sic'])
...
for gvkey in df['gvkey'].unique():
    ...
    if len(ok_data) > 0:
        ...
        df.loc[first_idx, 'BrandCapital'] = first_xad / (0.5 + 0.1)  # 0.6 depreciation rate
...
print(f"Generated BrandInvest values for {expanded_df['BrandInvest'].notna().sum():,} observations")

# Clean up temporary columns
expanded_df = expanded_df.drop(columns=['OK', 'tempYear', 'FirstNMyear', 'tempxad', 
                                      'BrandCapital', 'l_BrandCapital', 'sic_numeric', 'month'])
