...
print("Downloaded {len(ibes_adj)} IBES EPS adjusted records")
...
print("Removed {initial_count - len(ibes_adj)} records with missing meanest")
...
print("After keeping last obs per month: {len(ibes_adj)} records")

# Save the data
ibes_adj.to_pickle("../pyData/Intermediate/IBES_EPS_Adj.pkl")

print("IBES EPS Adjusted data saved with {len(ibes_adj)} records")
print("Date range: {ibes_adj['time_avail_m'].min()} to {ibes_adj['time_avail_m'].max()}")
